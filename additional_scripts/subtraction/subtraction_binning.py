...
subtraction_list = FileFinder.getFullPath('list_subtraction_all_files.csv') # main list of files to be scaled and subtracted; folder must be on Mantid path
index_files_to_subtract = "70-131"                                                       # index(es) of pair to subtract
...
# otherwise it is taking output_file_name as a name for the output file, adding ".dat" at the end
    ...
    if len(replaced_name) > 0:
       sub_file_output_short = replaced_name + name_suffix + ".csv"
    elif len(name_suffix) > 0:
        sub_file_output_short = sample_file[0:(len(sample_file.strip())-4)] + name_suffix + ".csv"
    else:
        sub_file_output_short = sample_file[0:(len(sample_file.strip())-4)] + name_suffix + "_sub.csv"
