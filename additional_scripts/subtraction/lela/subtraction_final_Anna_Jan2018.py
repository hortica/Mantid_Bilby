...
subtraction_list = FileFinder.getFullPath('FileList_6115_09.csv') # main list of files to be scaled and subtracted; folder must be on Mantid path

index_files_to_subtract = "0"                                                        # index(es) of pair to subtract

print ''
print ''
print 'List of files: ', subtraction_list
...
# creating new file, where X, Y, ErrY are taken from scaled/subtracted data, but the Xerror - i.e. sigmaQ - are copied from the original sample data
    for i in range(number_of_bins_sample):
        #add lela to remove nans
        print subtracted_data.readY(0)[i]
        if str(subtracted_data.readY(0)[i])=='nan':
            print 'nan, skipped line: ', i
            pass
        else:    
    ...
    print sub_file_output   
    #LoadAscii("BBY0014671_P3HT_h-CF_2mm.csv")    
    #LoadAscii("BBY0014612_P3HT_h-CB_TOF_7m_sub.dat", Separator = ',', SkipNumLines = '4')
    LoadAscii(sub_file_output, SkipNumLines = 4)#, SkipNumLines = 4)
    #a = LoadAscii('BBY0014612_P3HT_h-CB_TOF_7m_sub.dat')
    #LoadAscii(Separator = ',', Unit = 'MomentumTransfer', SkipNumLines = '4')
